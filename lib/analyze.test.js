...
test("retorna analise estruturada quando a IA responde JSON valido", async () => {
  const fakePayload = {
    ...
    suggestedNextSteps: ["Conferir a bula oficial na Anvisa."],
    finalDisclaimer:
  ...
  assert.equal(result.analysis.imageQuality.legibility, "boa");
  assert.deepEqual(result.analysis.suggestedNextSteps, [
    "Conferir a bula oficial na Anvisa."
  ]);
});
...
test("service adiciona fontes oficiais e reaproveita cache", async () => {
  const fakePayload = {
    ...
    imageQuality: {
      legibility: "regular",
      issues: ["Texto pequeno em parte da embalagem."]
    },
    seekProfessionalHelp: ["Procure atendimento em caso de reacao alergica."],
    suggestedNextSteps: ["Refazer a foto se a duvida continuar."],
    finalDisclaimer:
      "Este resumo e informativo e nao substitui consulta com medico ou outro profissional de saude."
  };
  let fetchCalls = 0;
...
test("prompt inclui pistas opcionais do usuario e qualidade local", async () => {
  let requestBody;
  ...
  await analyzeMedicineImage({
    ...
    hints: {
      ...
      notes: "Foto do blister e da caixa",
      barcodeValues: ["7891234567890"],
      imageQuality: {
        legibility: "regular",
        issues: ["Texto pequeno"]
      }
    },
                ...
                parts: [
                  {
                    text: JSON.stringify({
                      identifiedMedicine: {
                        ...
                        pharmaceuticalForm: "comprimido",
                        confidence: "alta",
                        confidenceReason: "Nome e dosagem legiveis."
                      },
                      evidenceInImage: ["Dipirona", "500 mg"],
                      bulaResumo: {
                        confidenceNote: "Resumo conservador.",
                        commonUses: [],
                        generalCare: [],
                        warnings: [],
                        commonSideEffects: [],
  ...
  assert.match(promptText, /nome informado pelo usuario: Dipirona/);
  assert.match(promptText, /Laboratorio Exemplo/);
  assert.match(promptText, /7891234567890/);
  assert.match(promptText, /legibilidade regular/);
});
